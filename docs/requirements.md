...
2. **Профиль пользователя**
   ...
   - Система должна позволять пользователю редактировать свои личные данные: имя, фамилию, отчество, email, telegramID.
   - Система должна обеспечивать принадлежность, записанного telegramID пользователю.
...
3. **Работа с чатами**
   ...
   - Система должна обеспечивать возможность отправки telegram-бота на "разведку" желаемого подарка у определенного пользователя.
...
- Система должна быть доступна пользователям большую часть времени без простоев. (uptime ≥ 99%).
- Сервер должен обрабатывать запросы и выдавать ответ в течение ≤ 1 секунды.
- Все пользовательские данные (включая токены и личную информацию) должны быть защищены при передаче и хранении.
- Система должна корректно работать на различных операционных системах и устройствах.
- Пользовательский интерфейс должен быть простым и понятным, локализованным на русский язык.
...
1. **Архитектурный стиль** — клиент-сервер с REST API.
   ...
   - База данных: PostgreSQL, Redis, Chroma.
